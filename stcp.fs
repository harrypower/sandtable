...
: processhttp ( "ccc" -- ) \ this is called from inetd and will simply get the stdin message sent from inetd and return a message
  try
    ...
    *http* to http?cmdline?
    s\" echo \"command=fastcalibration&xquick=0&yquick=0\" | nohup ( sudo --user=root --group=debian /home/debian/sandtable/stcp.fs -e \"processcmdline\" ) > stcpcmdlineout.data 2>&1 & "
