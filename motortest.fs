...
0x6c %00010000000000001000000010010011  mymotorx putreg . .
( diss2g 0, dedge 0, intpol 1, mres %0000, sync 0, vhighchm 0, vhighfs 0, vsense 0, TBL %01, chm 0, rndtf 0, disfdcc 0, TFD 0, HEND 1, HSTRT 1,TOFF %11 )
...
( TPWMTHRS )
0x70 %0111100000101011000000            mymotorx putreg . .
( freewheel %01 ,pwm_symmetric %1 ,pwm_autoscale %1 ,PWM freq %10 ,PWM_GRAD %1010 ,PWM_AMPL %11000000 )
0x6d %1011011000000000000000000         mymotorX putreg . .
( sfilt 1,sgt %1101100 , seimin 0, sedn 00, semax 0, seup 00, semin 0)
...
: sgt ( utime usteps -- )
  ...
  0x6f mymotorX getreg . dup dup %111111111 and . %1000000000000000000000000 and 0 = if ." N" else ." S" then %1000000000000000 and 0 = if ." R" else ." F" then . cr
